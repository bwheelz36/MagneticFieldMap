// Magnetic Field for TsMagneticFieldMap2
// ********************************************************************
// *                                                                  *
...
#include "TsMagneticFieldMap2.hh"
...
TsMagneticFieldMap2::TsMagneticFieldMap2(TsParameterManager* pM,TsGeometryManager* gM, TsVGeometryComponent* component):
TsVMagneticField(pM, gM, component), fInvertX(false), fInvertY(false), fInvertZ(false), fNX(0), fNY(0), fNZ(0) {
	ResolveParameters();
}


TsMagneticFieldMap2::~TsMagneticFieldMap2() {
	fFieldX.clear();
	fFieldY.clear();
	fFieldZ.clear();
...
void TsMagneticFieldMap2::ResolveParameters() {
	...
	while (file.good()) {
		...
		if ((thisRow[0] == "0") && (counter > 0)) {
			...
			for(G4int i = 0; i < (G4int)headerFields.size(); i++) {
				...
				if (unitString == "mm") {
					G4cerr << "multiplying field values by: " << mm << G4endl;
					...
					if (unitString == "m" || unitString == "metre" || unitString == "meter") {
						...
						if (unitString == "tesla") {
							...
							G4cerr << "multiplying field values by: " << tesla << G4endl;
						}
						else {
							headerUnits[headerFields[i]] = 1;
							G4cerr << "multiplying field values by 1" << G4endl;
						}
			}
...
void TsMagneticFieldMap2::GetFieldValue(const G4double Point[3], G4double* Field) const {
...
//		if (zIndex + 1 == fNZ) zIndex--;
		...
		G4cerr << " " << Point[0] << " " << Point[1] << " " << Point[2] << " " << FieldX << " " << FieldY << " " << FieldZ << G4endl;
